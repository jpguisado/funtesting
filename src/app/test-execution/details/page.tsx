...
import { fetchTestCaseByEnvironmentCycleAndId } from "@/server/data-layer"
...
export default async function Page({
    ...
    searchParams: Promise<{ id: string, env: string, cycleId: string }>
}) {
    const testCaseId = (await searchParams).id;
    const envId = (await searchParams).env;
    const cycleId = (await searchParams).cycleId;
    const testCaseDetails = await fetchTestCaseByEnvironmentCycleAndId(parseInt(testCaseId), parseInt(cycleId), parseInt(envId));
